...
# Function to get the center and radius of a circle
def get_circle(x, y):
...
# Function to get next possible nodes for Dijkstra
...
# Main game loop
while True:
    sc.blit(bg, (0, 0))  # Fill screen with background image

    # Draw Dijkstra work
    for x, y in visited:
        pg.draw.rect(sc, pg.Color('forestgreen'), get_rect(x, y), 1)
    for _, xy in queue:
    ...
    if queue:
        ...
        for next_node_cost, neigh_node in next_nodes:
            new_cost = cost_visited[cur_node] + next_node_cost
